...
import { enquiries, properties, homeDesigns, consultants } from "../db/schema";
...
export const enquiryService = {
    ...
    async getAll(filters = {}) {
        const conditions = [];
        if (filters.type) {
            conditions.push(eq(enquiries.type, filters.type));
        }
        if (filters.status) {
    ...
    async updateStatus(id, status, notes) {
        const result = await db.update(enquiries)
            .set({ status: status, notes, updatedAt: new Date() })

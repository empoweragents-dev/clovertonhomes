...
import { regions, estates, homeDesigns, properties, designImages, designFloorplans, propertyImages, user, account } from "./schema";
...
const main = async () => {
    ...
    try {
        ...
        const insertedRegions = await db.insert(regions).values(regionData.map(r => ({
            name: r.name,
        ...
        const sydneyEstates = insertedEstates.filter(e => ["Oran Park", "Leppington Living", "Gregory Hills", "Edmondson Park"].includes(e.name));

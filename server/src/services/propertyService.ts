...
export const propertyService = {
    ...
    async getAll(filters: PropertyFilters = {}): Promise<{ properties: Property[]; total: number }> {
        ...
        if (filters.badge) {
            conditions.push(eq(properties.badge, filters.badge as any));
        }
        if (filters.search) {
            conditions.push(
                or(
    ...
    // Create property
    async create(data: Omit<NewProperty, "id" | "slug" | "createdAt" | "updatedAt">): Promise<Property> {
    ...
    // Update property
    async update(id: string, data: Partial<NewProperty>): Promise<Property | undefined> {
        if (data.title) {
